...
export default function InputBox({ name, placeholder, value, onChange, disabled, textValuePassword, textValueEmail, textValueFullName, type }) {
    ...
    const handleOnBlur = () => {
        ...
        if (textValueFullName.length === 0 && type === "name") {
            setHelperTextName("Full Name is Required")
        }
        else {
            setHelperTextEmail("");
        }
    ...
    switch (type) {
        ...
        case 'Full Name':
            return (<div>
                ...
                <TextField onBlur={handleOnBlur} disabled={disabled} id="filled-basic" style={{ paddingBottom: "10px", width: "350px" }} label={placeholder} variant="filled" onChange={onChange} helperText={HelperTextName} />
